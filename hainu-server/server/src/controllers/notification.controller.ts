...
export async function submitFeedback(ctx: Context) { const { content, contact } = ctx.request.body as any; if (!content) throw new ApiError(40001, '缺少反馈内容'); ctx.body = success(await fs.submitFeedback(ctx.state.user.userId, content, contact)); }
