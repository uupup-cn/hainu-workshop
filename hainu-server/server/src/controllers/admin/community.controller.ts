...
// 校友圈帖子/评论
export async function listAlumniPosts(ctx: Context) { const { page, size } = parsePagination(ctx.query); const { type } = ctx.query as any; ctx.body = success(await cm.getAlumniPosts(page, size, type)); }
