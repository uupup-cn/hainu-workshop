...
<script setup lang="ts">
  ...
  import { applyLayoutDirection } from '@/hooks/core/useLayoutDirection'
  ...
  const ENUM_MAPS = {
    menuType: {
      ...
      'hover-menu': 'MenuTypeEnum.HOVER_MENU',
      sidebar: 'MenuTypeEnum.SIDEBAR',
      'content-full': 'MenuTypeEnum.CONTENT_FULL'
    },
    menuColorMode: {
      light: 'MenuColorModeEnum.LIGHT',
      dark: 'MenuColorModeEnum.DARK'
    },
    systemTheme: {
    ...
    layoutDirection: {
      ltr: 'LayoutDirectionEnum.LTR',
      rtl: 'LayoutDirectionEnum.RTL'
    }
  } as const
  ...
  const CONFIG_ITEMS: ConfigItem[] = [
    ...
    { comment: '菜单背景模式', key: 'menuColorMode', enumMap: ENUM_MAPS.menuColorMode },
    { comment: '是否手动覆盖菜单背景模式', key: 'menuColorModeOverridden' },
    { comment: '不同主题下的菜单背景模式偏好', key: 'menuColorModeByTheme' },
    ...
    { comment: '布局方向', key: 'layoutDirection', enumMap: ENUM_MAPS.layoutDirection },
    { comment: '是否显示进度条', key: 'showNprogress' },
    { comment: '是否显示节日文本', key: 'showFestivalText' },
    { comment: '是否显示水印', key: 'watermarkVisible' },
    { comment: '是否显示版权合规信息', key: 'showComplianceFooter' },
  ...
  const handleResetConfig = async () => {
    try {
      ...
      settingStore.setMenuOpen(isMobileViewport ? false : config.menuOpen)
      settingStore.setDualMenuShowText(config.dualMenuShowText)
      settingStore.setMenuColorModeByTheme(config.menuColorModeByTheme)
      settingStore.setMenuColorModeOverridden(config.menuColorModeOverridden)
      ...
      settingStore.setLayoutDirection(config.layoutDirection)
      applyLayoutDirection(config.layoutDirection)
      ...
      settingStore.setComplianceFooterVisible(config.showComplianceFooter)
      ...
      // 样式设置
      settingStore.setBoxBorderMode(config.boxBorderMode)
      ...
      settingStore.setThemeCustomizer(config.themeCustomizer)
      ...
      // 清掉响应式菜单在窄屏适配时写入的临时桌面菜单类型，避免重置后沿用旧布局
