...
import { ContainerWidthEnum, LayoutDirectionEnum } from '@/enums/appEnum'
...
export function useSettingsConfig() {
  ...
  const layoutDirectionOptions = computed(() => [
    {
      value: LayoutDirectionEnum.LTR,
      label: t('setting.direction.list[0]'),
      icon: 'ri:text-direction-l'
    },
    {
      value: LayoutDirectionEnum.RTL,
      label: t('setting.direction.list[1]'),
  ...
  const basicSettingsConfig = computed(() => {
    ...
    const allSettings = [
      ...
      {
        key: 'mobileTableCard',
        label: t('setting.basics.list.mobileTableCard'),
        type: 'switch' as const,
        handler: 'mobileTableCard',
        headerBarKey: null // 不依赖headerBar配置
      },
      {
        key: 'colorWeak',
      ...
      },
      {
        ...
        type: 'switch' as const,
        handler: 'watermark',
        disabled: !siteSettingsStore.watermarkEnabled,
        tip: siteSettingsStore.watermarkEnabled
          ? ''
          : t('setting.basics.list.watermarkDisabledTip'),
        headerBarKey: null // 不依赖headerBar配置
      },
      {
        key: 'showComplianceFooter',
        label: t('setting.basics.list.complianceFooter'),
        type: 'switch' as const,
        handler: 'complianceFooter',
        headerBarKey: null // 不依赖headerBar配置
      },
      {
        key: 'containerWidth',
        label: t('setting.container.title'),
        type: 'select' as const,
        handler: 'containerWidth',
        options: containerWidthOptions.value,
        style: { width: '120px' },
        headerBarKey: null
      },
      {
