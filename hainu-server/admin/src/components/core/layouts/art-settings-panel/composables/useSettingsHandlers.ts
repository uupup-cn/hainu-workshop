...
export function useSettingsHandlers() {
  ...
  const basicHandlers = {
    ...
    // 布局方向
    layoutDirection: createValueHandler<LayoutDirectionEnum>(
      (direction: LayoutDirectionEnum) => settingStore.setLayoutDirection(direction),
      (direction: LayoutDirectionEnum) => applyLayoutDirection(direction)
    ),
    ...
    // 移动端表格卡片模式
    mobileTableCard: createValueHandler<boolean>((value: boolean) =>
      tableStore.setIsMobileCardMode(value)
    ),
    ...
    // 版权合规信息显示
    complianceFooter: createToggleHandler(() =>
      settingStore.setComplianceFooterVisible(!settingStore.showComplianceFooter)
    ),
    ...
    // 容器宽度
    containerWidth: createValueHandler<ContainerWidthEnum>((type: ContainerWidthEnum) => {
  ...
  const boxStyleHandlers = {
    ...
    setBoxMode: (type: 'border-mode' | 'shadow-mode') => {
      settingStore.setBoxBorderMode(type === 'border-mode')
    }
  }
