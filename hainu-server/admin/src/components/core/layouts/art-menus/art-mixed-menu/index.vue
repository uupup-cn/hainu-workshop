...
<template>
  <div class="relative box-border flex-c w-full overflow-hidden">
    <!-- 左侧滚动按钮 -->
    <div v-show="showLeftArrow" class="button-arrow left-2" @click="scroll('left')">
      <ElIcon>
        <ArrowLeft />
      </ElIcon>
    </div>
      ...
      <div class="box-border flex-c flex-shrink-0 flex-nowrap h-15 whitespace-nowrap">
        <template v-for="item in processedMenuList" :key="item.meta.title">
          ...
          >
            <ArtSvgIcon
              :icon="item.meta.icon"
              class="text-lg text-g-700 dark:text-g-800 me-1"
...
<script setup lang="ts">
  ...
  const handleScrollCore = (): void => {
    ...
    if (isRtl.value) {
      // RTL: scrollLeft 从 0（起始/右端）到 -maxScroll（末端/左端）
      showLeftArrow.value = Math.abs(scrollLeft) < maxScroll - 1
      showRightArrow.value = scrollLeft < -1
    } else {
      // LTR: scrollLeft 从 0（起始/左端）到 maxScroll（末端/右端）
  ...
  const handleWheel = (event: WheelEvent): void => {
    ...
    let targetScroll
    if (isRtl.value) {
      // RTL: scrollLeft 范围 [-maxScroll, 0]
      targetScroll = Math.max(-maxScroll, Math.min(scrollLeft + scrollDelta, 0))
    } else {
      // LTR: scrollLeft 范围 [0, maxScroll]
      targetScroll = Math.max(0, Math.min(scrollLeft + scrollDelta, maxScroll))
    }
