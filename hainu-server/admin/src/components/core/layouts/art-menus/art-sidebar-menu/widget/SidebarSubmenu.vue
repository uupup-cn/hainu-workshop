  ...
  <template v-for="(item, index) in filteredMenuItems" :key="getUniqueKey(item, index)">
    <ElSubMenu v-if="hasChildren(item)" :index="item.path || item.meta.title" :level="level">
      <template #title>
        ...
        <div v-if="item.meta.showBadge" class="art-badge" :style="getInlineEndStyle('10px')" />
      </template>
...
<script setup lang="ts">
  ...
  const { menuOpen, isRtl } = storeToRefs(settingStore)
  const getInlineEndStyle = (offset: string) =>
    isRtl.value ? { left: offset, right: 'auto' } : { right: offset, left: 'auto' }
