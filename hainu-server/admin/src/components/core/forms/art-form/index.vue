...
<template>
  <section class="px-4 pb-0 pt-4 md:px-4 md:pt-4">
    <ElForm
      ...
      v-bind="{ ...$attrs }"
    >
      <div class="art-form-sections">
        <section
          v-for="section in formSections"
          :key="section.key"
          class="art-form-section"
          :class="{ 'art-form-section--grouped': section.label }"
        >
          <div v-if="section.label" class="art-form-section__title">
            {{ section.label }}
          </div>
...
<script setup lang="ts">
  ...
  const rootProps = ['label', 'labelWidth', 'key', 'type', 'hidden', 'required', 'span', 'slots']
  ...
  const formSections = computed(() => {
    if (!props.groups.length) {
      return [{ key: '__default', label: '', items: visibleFormItems.value }]
    }

    const itemMap = new Map(visibleFormItems.value.map((item) => [item.key, item]))
    ...
    const ungroupedItems = visibleFormItems.value.filter((item) => !usedKeys.has(item.key))
  ...
  defineExpose({
    ref: formInstance,
    validate: (...args: any[]) => formInstance.value?.validate(...args) ?? Promise.resolve(false),
    validateField: (...args: any[]) =>
      formInstance.value?.validateField(...args) ?? Promise.resolve(false),
...
<style scoped>
  .art-form-section--grouped + .art-form-section--grouped,
  .art-form-section--grouped + .art-form-section:not(.art-form-section--grouped),
  .art-form-section:not(.art-form-section--grouped) + .art-form-section--grouped {
    padding-top: 2px;
    margin-top: 20px;
  }

  .art-form-section__title {
    display: inline-flex;
    align-items: center;
    margin-bottom: 14px;
    font-size: 14px;
  ...
  .art-form-section__title::before {
    width: 3px;
    height: 14px;
    margin-right: 8px;
    content: '';
