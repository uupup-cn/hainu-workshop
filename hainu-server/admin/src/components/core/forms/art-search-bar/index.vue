...
<script setup lang="ts">
  ...
  export interface SearchFormItem {
    ...
    /** 表单项的占位符文本 */
    placeholder?: string
    /** 选择类控件值变化后是否自动触发搜索，会覆盖 SearchBar 的 autoSearchOnChange */
    autoSearch?: boolean
  ...
  const rootProps = ['label', 'labelWidth', 'key', 'type', 'hidden', 'span', 'slots', 'autoSearch']
  const autoSearchTypes = new Set<string>([
    'select',
    'cascader',
    'treeselect',
    'date',
  ...
  const shouldAutoSearch = (item: SearchFormItem) => {
    if (item.autoSearch !== undefined) return item.autoSearch
    return props.autoSearchOnChange && autoSearchTypes.has(String(item.type))
  }

  const handleFieldUpdate = (item: SearchFormItem, value: unknown) => {
    setFieldValue(item.key, value)

    if (shouldAutoSearch(item)) {
      handleSearch()
    }
  }
  ...
  const visibleFormItems = computed(() => {
    ...
    if (shouldShowLess) {
      ...
      return filteredItems.filter((item) => {
        if (inlineItemCount >= maxItemsPerRow) return false

        const itemSpan = item.span || props.span
        if (itemSpan < 24) {
          inlineItemCount += 1
        }
  ...
  /**
   * 处理回车搜索，仅让普通输入框触发，避免干扰选择器、日期控件等组件的内部交互。
   */
  const handleEnterSearch = (event: KeyboardEvent) => {
    if (!props.enterSearch || props.disabledSearch || event.isComposing) return
