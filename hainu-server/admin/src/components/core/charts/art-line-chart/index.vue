...
<script setup lang="ts">
  ...
  const getColor = (customColor?: string, index?: number): string => {
    ...
    if (index !== undefined) return resolvedColors.value[index % resolvedColors.value.length]
  ...
  const generateChartOptions = (isInitial = false): EChartsOption => {
    ...
    if (isMultipleData.value) {
      ...
      options.series = multiData.map((item, index) => {
        const itemColor = getColor(undefined, index)
  ...
  watch([() => props.data, () => props.xAxisData, () => resolvedColors.value], renderChart, {
    deep: true
  })
