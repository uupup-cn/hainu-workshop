...
<template>
  <div class="art-card-sm p-5">
    ...
    <ElScrollbar :style="{ height: maxHeight }">
      <div v-for="(item, index) in list" :key="index" class="flex-c gap-3 py-3">
        <div v-if="item.icon" class="flex-cc size-10 rounded-lg" :class="item.class">
        ...
        <div class="text-xs text-g-500">{{ item.time }}</div>
      </div>
    </ElScrollbar>
    <ElButton
