...
<template>
  <div
    class="art-card-sm basic-banner"
    :class="[{ 'has-decoration': decoration, 'is-rtl': isRtl, 'is-ltr': !isRtl }, boxStyle]"
    :style="{ height }"
    @click="emit('click')"
  >
    <!-- 流星效果 -->
    ...
    <div class="basic-banner__content">
      ...
      <img
        ...
        :style="{
          ...
          '--banner-image-offset': imageConfig.right ?? '0'
        }"
        loading="lazy"
        alt="背景图片"
      />
...
<script setup lang="ts">
  ...
  const { isDark, isRtl } = storeToRefs(settingStore)
...
<style lang="scss" scoped>
  .basic-banner {
    ...
    overflow: hidden;
    color: white;
    border-radius: calc(var(--custom-radius)) !important;
    ...
    &__background-image {
      position: absolute;
      right: var(--banner-image-offset, 0);
      bottom: -3rem;
      left: auto;
      z-index: 0;
      width: 12rem;
    ...
    &.is-rtl &__background-image {
      right: auto;
      left: var(--banner-image-offset, 0);
    }
