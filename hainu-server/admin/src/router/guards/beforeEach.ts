...
import { restoreAccessToken } from '@/utils/http'
...
const LEGACY_AI_GENERATOR_PATH = '/system/ai-generator'
const AI_GENERATOR_PATH = '/dev-tools/ai-generator'

// 跟踪是否需要关闭 loading
let pendingLoading = false

// loading 开始时间，用于保证最短展示时长
let loadingStartTime = 0

export function getLoadingStartTime(): number {
...
async function handleRouteGuard(
  ...
  if (to.path === LEGACY_AI_GENERATOR_PATH) {
    next({
      path: AI_GENERATOR_PATH,
      query: to.query,
      hash: to.hash,
      replace: true
    })
...
async function handleDynamicRoutes(
  ...
  try {
    ...
    const targetAfterInit = getPendingNavigationTarget(to)

    // 8. 静态路由不依赖菜单权限，初始化后直接恢复目标地址。
    if (isStaticRoute(targetAfterInit.path)) {
      next(buildNavigationTarget(targetAfterInit))
      return
    }
...
function getPendingNavigationTarget(to: RouteLocationNormalized) {
  return (
    pendingNavigationTarget ?? {
      path: to.path,
      query: to.query,
...
function buildNavigationTarget(target: Pick<RouteLocationNormalized, 'path' | 'query' | 'hash'>) {
  pendingNavigationTarget = null
...
export function resetRouterState(delay: number): void {
  ...
  if (delay > 0) {
    resetRouterTimer = setTimeout(reset, delay)
    return
  }
