  ...
  <div>
    <ElCard shadow="never">
      ...
      <ArtTable :loading="loading" :data="tableData" :columns="columns">
        <ElTableColumn prop="colorName" label="颜色名称" width="150" />
        <ElTableColumn prop="colorValue" label="颜色值" width="120" />
...
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除？', '提示'); await api.fetchDeleteCourseColor(row.id); loadData() }
onMounted(loadData)
</script>