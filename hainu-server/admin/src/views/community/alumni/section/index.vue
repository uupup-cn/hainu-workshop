<template>
  <div>
    <ElCard shadow="never">
      <div class="mb-4"><ElButton type="primary" plain @click="openDialog()">新增版块</ElButton></div>
      <ArtTable :loading="loading" :data="tableData" :columns="columns">
        <ElTableColumn prop="sectionName" label="版块名称" width="200" />
    ...
    <ElDialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <ElForm :model="form" label-width="80px">
        <ElFormItem label="版块名称"><ElInput v-model="form.sectionName" /></ElFormItem>
...
async function loadData() { loading.value = true; try { tableData.value = (await api.fetchAlumniSections() ) } finally { loading.value = false } }
...
async function handleSave() { try { if (editId.value) await api.fetchUpdateAlumniSection(editId.value, form.value); else await api.fetchCreateAlumniSection(form.value); dialogVisible.value = false; loadData() } catch {} }
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除？', '提示'); await api.fetchDeleteAlumniSection(row.id); loadData() }
onMounted(loadData)
</script>