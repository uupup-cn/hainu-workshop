<template>
  <div>
    <ElRow :gutter="16" class="mb-3">
      <ElCol :span="6"><ElCard shadow="never"><div class="text-gray-500 text-sm">表白墙总数</div><div class="text-2xl font-bold mt-2">{{ stats.total }}</div></ElCard></ElCol>
    ...
    <ElCard shadow="never" style="margin-top: 12px">
      <ArtTable :loading="loading" :data="tableData" :columns="columns" :pagination="{ current: page, size, total }" @pagination:current-change="handlePage" @pagination:size-change="handleSize">
        <ElTableColumn label="用户" width="110"><template #default="{ row }">{{ row.isAnonymous ? '匿名' : (row.user?.nickname || row.user?.uid) }}</template></ElTableColumn>
        <ElTableColumn label="匿名" width="80"><template #default="{ row }"><ElTag :type="row.isAnonymous ? 'warning' : 'info'" size="small">{{ row.isAnonymous ? '是' : '否' }}</ElTag></template></ElTableColumn>
    ...
    <ElDialog v-model="previewVisible" title="表白墙预览" width="700px" destroy-on-close>
      <div v-if="previewRow">
        <div class="mb-2 text-gray-500 text-sm">用户：{{ previewRow.isAnonymous ? '匿名' : (previewRow.user?.nickname || previewRow.user?.uid) }} · {{ previewRow.isAnonymous ? '匿名' : '实名' }}</div>
    ...
    <ElDialog v-model="dialogVisible" title="编辑表白墙" width="800px" destroy-on-close>
...
const searchItems = [{ label: '关键词', key: 'keyword', type: 'input', props: { clearable: true, placeholder: '请输入内容关键词' } }]
...
async function loadData() {
  loading.value = true
  try { const res: any = await api.fetchAlumniPosts({ page: page.value, size: size.value, type: 'confession', keyword: searchForm.value.keyword || undefined }); tableData.value = res.list; total.value = res.total }
