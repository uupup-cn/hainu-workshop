...
async function loadData() { loading.value = true; try { tableData.value = (await api.fetchAdminMarketCategories()) as any[] } finally { loading.value = false } }
function openDialog(row?: any) { editId.value = row?.id || null; dialogTitle.value = row ? '编辑分类' : '新增分类'; form.value = row ? { ...row } : { isActive: true, sortOrder: 0 }; dialogVisible.value = true }
async function handleSave() { try { if (editId.value) await api.fetchUpdateMarketCategory(editId.value, form.value); else await api.fetchCreateMarketCategory(form.value); dialogVisible.value = false; loadData() } catch {} }
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除该分类？', '提示'); await api.fetchDeleteMarketCategory(row.id); loadData() }
onMounted(loadData)
</script>
