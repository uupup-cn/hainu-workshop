...
async function loadData() { loading.value = true; try { tableData.value = (await api.fetchIntroEntries() ) } finally { loading.value = false } }
...
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除？', '提示'); await api.fetchDeleteIntro(row.id); loadData() }
onMounted(loadData)
</script>