<template>
  <div>
    <ElCard shadow="never">
      <div class="mb-4"><ElButton type="primary" plain @click="openDialog()">新增楼栋</ElButton></div>
      <ArtTable :loading="loading" :data="tableData" :columns="columns">
        <ElTableColumn prop="buildingName" label="楼栋名称" width="200" />
        <ElTableColumn prop="collegeId" label="书院ID" width="100" />
    ...
    <ElDialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <ElForm :model="form" label-width="80px">
        <ElFormItem label="楼栋名称"><ElInput v-model="form.buildingName" /></ElFormItem>
        <ElFormItem label="书院ID"><ElInput v-model="form.collegeId" /></ElFormItem>
...
async function loadData() { loading.value = true; try { tableData.value = (await api.fetchBuildings() ) } finally { loading.value = false } }
...
async function handleSave() { try { if (editId.value) await api.fetchUpdateBuilding(editId.value, form.value); else await api.fetchCreateBuilding(form.value); dialogVisible.value = false; loadData() } catch {} }
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除？', '提示'); await api.fetchDeleteBuilding(row.id); loadData() }
onMounted(loadData)
</script>