<template>
  <div>
    ...
    <ElDialog
      ...
      :title="dialogMode === 'add' ? '新增学院' : '编辑学院'"
      width="800px"
      destroy-on-close
    >
      <ElForm ref="formRef" :model="form" :rules="rules" label-width="80px">
        <ElFormItem label="学院名称" prop="departmentName">
          <ElInput v-model.trim="form.departmentName" maxlength="50" placeholder="请输入学院名称" />
        </ElFormItem>
        <ElFormItem label="排序" prop="sortOrder">
          <ElInputNumber v-model="form.sortOrder" :min="0" />
        </ElFormItem>
        <ElFormItem label="学院介绍" prop="description">
          <ArtWangEditor
...
<script setup lang="ts">
  ...
  const columns = ref<ColumnOption[]>([
    { prop: 'departmentName', label: '学院名称', width: 200 },
  ...
  const form = reactive<Department>(createForm())
  ...
  async function loadData() {
    loading.value = true
    try {
      tableData.value = ((await api.fetchDepartments()) as Department[]) || []
    } finally {
      loading.value = false
    }
  }
  ...
  async function handleSave() {
    ...
    try {
      if (editId.value) {
        await api.fetchUpdateDepartment(editId.value, { ...form })
      } else {
        await api.fetchCreateDepartment({ ...form })
      }
  ...
  async function handleDelete(row: Department) {
    if (!row.id) return
    await ElMessageBox.confirm(`确认删除学院"${row.departmentName}"？`, '提示', { type: 'warning' })
    await api.fetchDeleteDepartment(row.id)
