<template>
  <div>
    <ElCard shadow="never">
      <div class="mb-4"><ElButton type="primary" plain @click="openDialog()">新增专业</ElButton></div>
      <ArtTable :loading="loading" :data="tableData" :columns="columns">
        <ElTableColumn prop="majorName" label="专业名称" width="200" />
        <ElTableColumn prop="departmentId" label="学院ID" width="100" />
    ...
    <ElDialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <ElForm :model="form" label-width="80px">
        <ElFormItem label="专业名称"><ElInput v-model="form.majorName" /></ElFormItem>
        <ElFormItem label="学院ID"><ElInput v-model="form.departmentId" /></ElFormItem>
...
async function loadData() { loading.value = true; try { tableData.value = (await api.fetchMajors() ) } finally { loading.value = false } }
...
async function handleSave() { try { if (editId.value) await api.fetchUpdateMajor(editId.value, form.value); else await api.fetchCreateMajor(form.value); dialogVisible.value = false; loadData() } catch {} }
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除？', '提示'); await api.fetchDeleteMajor(row.id); loadData() }
onMounted(loadData)
</script>