  ...
  <div>
    <ElCard shadow="never">
      ...
      <ArtTable :loading="loading" :data="tableData" :columns="columns">
        <ElTableColumn prop="campusName" label="校区名称" width="150" />
    ...
    <ElDialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <ElForm :model="form" label-width="80px">
        <ElFormItem label="校区名称"><ElInput v-model="form.campusName" /></ElFormItem>
...
async function loadData() { loading.value = true; try { tableData.value = (await api.fetchCampuses() ) } finally { loading.value = false } }
...
async function handleSave() { try { if (editId.value) await api.fetchUpdateCampus(editId.value, form.value); else await api.fetchCreateCampus(form.value); dialogVisible.value = false; loadData() } catch {} }
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除？', '提示'); await api.fetchDeleteCampus(row.id); loadData() }
onMounted(loadData)
</script>