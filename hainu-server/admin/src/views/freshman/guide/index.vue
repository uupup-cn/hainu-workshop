  ...
  <div>
    <ElCard shadow="never">
      ...
      <ArtTable :loading="loading" :data="tableData" :columns="columns">
        <ElTableColumn prop="entryKey" label="标识" width="120" />
    ...
    <ElDialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <ElForm :model="form" label-width="80px">
        <ElFormItem label="标识"><ElInput v-model="form.entryKey" /></ElFormItem>
        <ElFormItem label="标题"><ElInput v-model="form.entryTitle" /></ElFormItem>
...
async function loadData() { loading.value = true; try { tableData.value = (await api.fetchGuideEntries() ) } finally { loading.value = false } }
...
async function handleSave() { try { if (editId.value) await api.fetchUpdateGuide(editId.value, form.value); else await api.fetchCreateGuide(form.value); dialogVisible.value = false; loadData() } catch {} }
async function handleDelete(row: any) { await ElMessageBox.confirm('确认删除？', '提示'); await api.fetchDeleteGuide(row.id); loadData() }
onMounted(loadData)
</script>