...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(
  ...
  // 历史行为只更新参数不发请求，导致很多页面"切换筛选条件无效"。
  // 现在统一在替换参数后清空当前缓存并重新拉取，调用方无需再手动 await getData()。
  const replaceSearchParams = async (params?: Partial<TParams>): Promise<void> => {
    debouncedGetDataByPage.cancel()
    ...
    // 切换筛选条件时清空当前页缓存，确保拿到最新结果
    clearCache(CacheInvalidationStrategy.CLEAR_CURRENT, '替换搜索参数')
