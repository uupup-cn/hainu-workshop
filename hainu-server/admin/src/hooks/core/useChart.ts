...
import {
  echarts,
  ensureEChartsModules,
  refreshEChartsTheme,
  resolveEChartsModules,
  type EChartsOption
} from '@/plugins/echarts'
import { storeToRefs } from 'pinia'
import { useSettingStore } from '@/store/modules/setting'
import { getCssVar, getThemeChartPalette } from '@/utils/ui'
import type { BaseChartProps, ChartThemeConfig, UseChartOptions } from '@/types/component/chart'

const createChartThemeColors = () =>
  new Proxy(getThemeChartPalette(), {
    get(target, key, receiver) {
      if (typeof key === 'string' && /^[0-5]$/.test(key)) {
        return getCssVar(`--chart-color-${Number(key) + 1}`) || target[Number(key)]
      }
...
export const useChartOps = (): ChartThemeConfig => ({
  ...
  fontSize: 13,
  /** 字体颜色 */
  fontColor: getCssVar('--art-gray-800'),
  /** 主题颜色 */
  themeColor: getCssVar('--el-color-primary-light-1'),
  /** 颜色组 */
  colors: createChartThemeColors()
})
...
export function useChart(options: UseChartOptions = {}) {
  ...
  const { isDark, menuOpen, menuType, systemThemeColor, isRtl, themeCustomizer } =
  ...
  const ensureChartModules = (options: EChartsOption) =>
    ensureEChartsModules(resolveEChartsModules(options, requiredModules))
  ...
  const setupThemeWatcher = () => {
    if (autoTheme) {
      themeStopHandle = watch([isDark, systemThemeColor, isRtl], () => {
        // 更新空状态样式
  ...
  const getLegendStyle = (
    ...
    const resolvedPosition =
      position === 'left' && isRtl.value
        ? 'right'
        : position === 'right' && isRtl.value
          ? 'left'
          : position
  ...
  const getGridWithLegend = (
    ...
    const defaultGrid = {
      top: 15,
      right: isRtl.value ? 0 : 15,
      bottom: 8,
      left: isRtl.value ? 15 : 0,
    ...
    switch (legendPosition) {
      ...
      case 'left':
        return {
          ...defaultGrid,
          left: isRtl.value ? defaultGrid.left : 120,
          right: isRtl.value ? 120 : defaultGrid.right
        }
      case 'right':
        return {
          ...defaultGrid,
          right: isRtl.value ? defaultGrid.right : 120,
          left: isRtl.value ? 120 : defaultGrid.left
        }
      default:
        return defaultGrid
    }
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting && pendingOptions && !isDestroyed) {
            ...
            requestAnimationFrame(() => {
              ...
              if (!isDestroyed && nextOptions) {
                void ensureChartModules(nextOptions)
                  .then(() => {
                    if (isDestroyed || !nextOptions) {
                      return
                    }
  ...
  const performChartInit = async (options: EChartsOption) => {
    ...
    await ensureChartModules(options)

    if (isDestroyed || requestId !== chartRequestId) {
      return
    }
  ...
  const initChart = (options: EChartsOption = {}, isEmpty: boolean = false) => {
    ...
    try {
      ...
      if (isContainerVisible(chartRef.value)) {
        ...
        if (initDelay > 0) {
          setTimeout(() => {
            void performChartInit(mergedOptions).catch((error) => {
              console.error('图表初始化失败:', error)
            })
          }, initDelay)
        } else {
  ...
  const updateChart = (options: EChartsOption) => {
    ...
    try {
      ...
      void ensureChartModules(options)
        .then(() => {
          if (!chart || isDestroyed) {
            return
          }
...
export function useChartComponent<T extends BaseChartProps>(options: UseChartComponentOptions<T>) {
  ...
  const chartThemeColorsSignature = computed(() => themeCustomizer.value.chartThemeColors.join('|'))
  ...
  const setupWatchers = () => {
    ...
    // 监听主题变化
    const themeStopHandle = watch(
      [isDark, systemThemeColor, isRtl, chartThemeColorsSignature],
      () => {
        emptyStateManager.updateStyle()
        updateChart()
        nextTick(refreshEChartsTheme)
      }
    )
