...
export const handleMenuJump = (item: AppRouteRecord, jumpToFirst: boolean = false) => {
  ...
  if (!jumpToFirst || !item.children?.length) {
    preloadMenuComponent(item)
    if (item.meta.openInNewTab) {
      return openRouteInNewTab(item.path)
    }
  ...
  // 跳转到子菜单路径
  preloadMenuComponent(firstChild)
  if (firstChild.meta.openInNewTab) {
    return openRouteInNewTab(firstChild.path)
  }
  router.push(firstChild.path)
}

const preloadMenuComponent = (item: AppRouteRecord): void => {
  const componentPath = typeof item.component === 'string' ? item.component : ''
  if (!componentPath) {
    return
  }

  void componentLoader.preload(componentPath)
}
