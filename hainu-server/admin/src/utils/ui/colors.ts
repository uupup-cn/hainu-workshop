 ...
 * @returns CSS变量值
 */
export const getCssVar = readRootCssVar
...
export function hexToRgb(hexColor: string): [number, number, number] {
  const hexPairs = normalizeHexColor(hexColor).match(/../g)
  if (!hexPairs) {
    throw new Error('Invalid hex color format')
  }

  return hexPairs.map((hexPair) => parseInt(hexPair, 16)) as [number, number, number]
}
...
export function handleElementThemeColor(theme: string, isDark: boolean = false): void {
  writeRootCssVar('--el-color-primary', theme)

  for (let i = 1; i <= 9; i++) {
    writeRootCssVar(`--el-color-primary-light-${i}`, getLightColor(theme, i / 10, isDark))
  }

  for (let i = 1; i <= 9; i++) {
    writeRootCssVar(`--el-color-primary-dark-${i}`, getDarkColor(theme, i / 10))
  }
}
...
export function setElementThemeColor(color: string): void {
  ...
  writeRootCssVar('--el-color-primary', color)
  writeRootCssVars(THEME_PRIMARY_CSS_VAR_NAMES, color)
  handleElementThemeColor(color, useSettingStore().isDark)

  // 生成更淡一点的颜色
  for (let i = 1; i < 16; i++) {
    const itemColor = colourBlend(color, mixColor, i / 16)
    writeRootCssVar(`--el-color-primary-custom-${i}`, itemColor)
  }
}
