...
export const useWorktabStore = defineStore(
  ...
  () => {
    ...
    /**
     * 检查标签页是否允许移动到目标位置
     * 固定标签和普通标签只允许在各自分区内拖拽排序
     */
    const canMoveTab = (fromIndex: number, toIndex: number): boolean => {
      if (
        fromIndex < 0 ||
        toIndex < 0 ||
        fromIndex >= opened.value.length ||
        toIndex >= opened.value.length
      ) {
        return false
      }

      return (
        !!opened.value[fromIndex] &&
        !!opened.value[toIndex] &&
        !!opened.value[fromIndex].fixedTab === !!opened.value[toIndex].fixedTab
      )
    }

    /**
     * 根据路径顺序重排标签页
     * 会自动修正固定标签分区，避免拖拽或异常状态破坏标签页结构
     */
    const reorderTabs = (paths: string[]): void => {
      if (paths.length !== opened.value.length) {
        console.warn('标签页重排失败：路径数量不匹配')
        return
      }

      const uniquePaths = new Set(paths)
      if (uniquePaths.size !== paths.length) {
        console.warn('标签页重排失败：存在重复路径')
        return
      }

      const tabMap = new Map(opened.value.map((tab) => [tab.path, tab]))
      const orderedTabs = paths
        .map((path) => tabMap.get(path))
        .filter((tab): tab is WorkTab => !!tab)

      if (orderedTabs.length !== opened.value.length) {
        console.warn('标签页重排失败：存在无效路径')
        return
      }

      const fixedTabs = orderedTabs.filter((tab) => tab.fixedTab)
      const normalTabs = orderedTabs.filter((tab) => !tab.fixedTab)
      opened.value = [...fixedTabs, ...normalTabs]

      if (current.value.path) {
        const currentTab = opened.value.find((tab) => tab.path === current.value.path)
        if (currentTab) {
