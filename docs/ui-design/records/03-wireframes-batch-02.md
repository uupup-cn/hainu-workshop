...
6. 智慧海大主页 <SmartHNU>
进入路径:首页底部 TabBar 第二位 页面用途:智慧海大模块聚合入口（6 主入口）

布局说明
从上到下 3 个区块:

NavBar default
智慧海大主入口 2×3 Grid（6 项）
TabBar
线框
┌──────────────────────────────────────┐
│  [←]      智慧海大      🔔 ●        │   NavBar default + Badge
├──────────────────────────────────────┤
│                                      │
│  ┌─ 智慧海大 ──────────────────┐   │   Section title h4
│  │                                │   │
│  │ [海大介绍] [电话簿] [校历]    │   │ ← 2×3 Grid(图标 32)
│  │                                │   │
│  │ [校园地图] [校园出行] [校园数据库]│   │
│  │                                │   │
│  │  ↑ 图标底 primary-50 圆        │   │
...
入口网格	<Card> 包裹自定义网格	bg bg-card, radius-xl 16, shadow-card
入口单格	圆 48 + primary-50 底 + primary-500 图标 24 + caption	primary-50, primary-500

颜色配比自检
颜色	用途	占比(估)
中性色 / bg	卡片底、底色、文字	约 82%
primary-50	入口图标底	约 14%
primary-500	图标、链接	约 4%
mint	0%	0%
orange	0%	0%
主色 18%,薄荷 0%,橙 0% ✓
...
「Badge 变色」说明:默认 Badge 使用 danger 红色表示未读/错误/预警；若用于「积极进行中的任务」建议按需改用 mint-500（页面层级调色，不改变 Badge 默认色）。

周视图课表大网格:水平 7 列 + 垂直时间轴,在 375 宽度上每列 ≈ 50px,课程格最小高度 64。窄屏下可以加横向滚动或切换到日视图(下一批)。本批线框以标准宽度为基准。

如果这批 8 个页面没问题:

「继续」 → 我出第三批:海大介绍 / 电话簿 / 校历 / 校园地图 / 校园出行 / 校园数据库 + 课表日视图 + 课程详情 + 个人中心 + 消息中心 + 发布器
「改 XXX」 → 指出页面或区块,就地改
