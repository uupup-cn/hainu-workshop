...
- 本文件只保存**真实可复用组件**。
- 线框记录中出现的 `FreshmanHub / SmartHNU / ScheduleWeek / Market / AboutDetail` 等 `<页面名>` 为页面模板，**不进入组件库**。
- 步骤条、时间轴、手风琴、FAB、图片上传预览、地图图层、周次网格等一律用既有组件组合实现，不新增组件。
- 一卡通、空教室、成绩/进度相关组件**不进入组件库**（真实设计文档未定义这些页面）。

## 新增组件

### 9.1 RichText（富文本渲染）

- 用途：海大介绍详情 / 乘车指南 / 快讯详情 / 消息详情 / 帖子详情
- 支持：标题（h2/h3/h4）、段落、列表、内嵌图片、引用
- Token：
  - 标题 `text-primary` + h3/h4
  - 正文 `body 14/22/400` + `text-primary`
  - 图片 `radius-md 8` + `shadow-xs`
  - 引用块 `neutral-100 bg` + `border-light`
- 默认最大宽度：容器宽度；图片支持点击放大（配合 ImagePreview）

### 9.2 ImageUploader（多图上传）

- 用途：发帖/发表白墙（最多 9 张）、认证材料（最多 3 张）、找室友证明、反馈截图
- 属性：
  - `maxCount`（3 / 9）
  - `accept`（image）
  - `onUpload` / `onRemove`
- 视觉：
  - 占位格：80×80 `radius-md 8` + `border-default`
  - 已上传：图片缩略 + 删除圆角角标
  - 当前上传中：进度覆盖层 + `Loading`
- Token：`primary-50`（上传按钮占位）、`danger`（删除角标）、`radius-md`

### 9.3 ImagePreview（图片预览 / 轮播）

- 用途：单图/多图放大预览、商品图轮播、校园风光瀑布流全屏查看
- 交互：
  - 双指缩放 100% / 150% / 200%
  - 单指拖动
  - 多图时支持横向轮播 + 指示器（圆点 + `当前 n / N`）
- Token：`bg-mask rgba(0,0,0,0.5)`、指示器 `primary-500`（选中）/ `neutral-300`（未选中）

### 9.4 DatePickerCell（日期/周次节次选择）

- 用途：校历日历视图、课表周视图、日期/节次筛选、发布时间选择
- 交互：
  - 网格型选择（月历 7×6 / 周次 7 列）
  - 支持颜色圆点标注（primary / mint / orange / danger）
  - 点击弹 `Popover` 显示详情
- Token：`neutral-300` 未选中、`primary-500` 选中、`primary-50` 选中底、`tnum` 数字
